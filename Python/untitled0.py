...
with open('example.csv', newline='') as myFile:  
    reader = csv.reader(myFile)
    for row in reader:
        print(row)
        
import csv
writer = csv.writer(open("example.csv", "w"), quoting=csv.QUOTE_NONE,escapechar='')
reader = csv.reader(open("example7.csv", "r"), skipinitialspace=True)
...
myData = [[5, 8, 9], ['vijay', 'ajay', 'ravi "addrsss" ']]  
